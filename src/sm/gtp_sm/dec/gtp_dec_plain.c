...
{
  // Remove sizeof(gtp_ho_info_t*) since ho_info is embedded struct, not a pointer
  assert(next_pow2(len) >= sizeof(gtp_ind_msg_t) - sizeof(gtp_ngu_t_stats_t*)
         && "Less bytes than the case where there are not active Radio bearers! Next pow2 trick used for aligned struct");

  gtp_ind_msg_t ret = {0};

  // Input validation
  assert(ind_msg != NULL && "Input message buffer is NULL");
