...
static const char* convert_period(Interval inter_arg)
{
...
static slice_cb* hndlr_slice_cb;
  ...
  if (ctrl->type == SLICE_CTRL_SM_V0_ADD) {
    ...
    for (size_t i = 0; i < s_conf->dl.len_slices; ++i) {
      ...
      if (p->type == SLICE_ALG_SM_V0_STATIC) {
        ...
        std::cout << "ADD STATIC DL SLICE: id " << s->id << ", label " << s->label << ", pos_low " << sta_sli->pos_low
        ...
        if (nvs_sli->conf == SLICE_SM_NVS_V0_RATE)
          std::cout << "ADD NVS DL SLICE: id " << s->id << ", label " << s->label << ", conf " << nvs_sli->conf << "(rate)"
                    << ", mbps_required " << nvs_sli->u.rate.u1.mbps_required << ", mbps_reference "
                    << nvs_sli->u.rate.u2.mbps_reference << '\n';
        else if (nvs_sli->conf == SLICE_SM_NVS_V0_CAPACITY)
          std::cout << "ADD NVS DL SLICE: id " << s->id << ", label " << s->label << ", conf " << nvs_sli->conf << "(capacity)"
                    << ", pct_reserved " << nvs_sli->u.capacity.u.pct_reserved << '\n';
        else
          assert(0 != 0 && "Unknow NVS conf");
      } else if (p->type == SLICE_ALG_SM_V0_EDF) {
        edf_slice_t* edf_sli = &p->u.edf;
        std::cout << "ADD EDF DL SLICE: id " << s->id << ", label " << s->label << ", deadline " << edf_sli->deadline
                  << ", guaranteed_prbs " << edf_sli->guaranteed_prbs << ", max_replenish " << edf_sli->max_replenish << '\n';
      } else
        assert(0 != 0 && "Unknow slice algo type");
    }
  } else if (ctrl->type == SLICE_CTRL_SM_V0_UE_SLICE_ASSOC) {
    for (size_t i = 0; i < ctrl->u.ue_slice.len_ue_slice; ++i)
      std::cout << "ASSOC DL SLICE: rnti " << std::hex << ctrl->u.ue_slice.ues[i].rnti << ", to slice id "
                << ctrl->u.ue_slice.ues[i].dl_id << '\n';
...
static void sm_cb_gtp(sm_ag_if_rd_t const* rd)
{
  ...
  // Copy Handover Information
  ind.ho_info.ue_id = data->msg.ho_info.ue_id;
  ind.ho_info.source_du = data->msg.ho_info.source_du;
  ind.ho_info.target_du = data->msg.ho_info.target_du;
  ind.ho_info.ho_complete = data->msg.ho_info.ho_complete;
